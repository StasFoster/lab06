...
#pragma endregion
	private: System::Void buttonTaskSolve_Click(System::Object^  sender, System::EventArgs^  e) 
         ...
         libptr = LoadLibrary(TEXT("clrSHASModuleLib.dll"));
           if (libptr != 0)
           {
             modptr = (MYPROC)GetProcAddress(libptr,"SHASmodule");
         if (modptr != 0)
         {
           iRet = (modptr)(ptr);
         }
         ...
         {
           Windows::Forms::MessageBox::Show("SHASmodule do not correct execute!");
         }
           }
           else
           {
             Windows::Forms::MessageBox::Show("Library 'clrSHASModuleLib.dll' not found! Operation do not execute!");
           }
           FreeLibrary(libptr);
           libptr = 0;
